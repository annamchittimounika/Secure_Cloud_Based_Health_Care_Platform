...
public class doctor_reg extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
            ...
            String sqlQuery = "insert into doctorregister(name,password,email,mobile,address,dob,gender,pincode,location,status,accesskey) values(?,?,?,?,?,?,?,?,?,?,?)";
            ...
            if(no>0){
                response.sendRedirect("doctorreg.jsp?msg=success");
            }else{
                response.sendRedirect("doctorreg.jsp?msg=failed");
            }
            
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("doctorreg.jsp?msg=exist");
        }finally{
            try{
                ps.close();
                con.close();
